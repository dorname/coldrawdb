...
main() {
    ...
    (
        cd frontend-rs
        # fix-select-bg-diagram-delete-and-log-format：强制 NO_COLOR=1，trunk 输出落盘日志不含 ANSI 转义
        # fix-local-start-and-invite-config：env 选项必须先于 NAME=VALUE 赋值（GNU env 参数顺序），
        # 否则 -u 被当作命令名、trunk 不会启动；trunk 把 NO_COLOR 解析为 bool（只接受 true/false），
        # 不能用 NO_COLOR=1。
        # 双进程 dev：trunk :8080 无代理，注入编译期 COLDRAWDB_API_BASE 使前端 API 直连后端
        # （core-01-deployment-plan §12.2/§12.4；生产单入口构建不得携带该变量）。
        exec env -u FORCE_COLOR NO_COLOR=true COLDRAWDB_API_BASE="http://127.0.0.1:${COLDRAWDB_BACKEND_PORT}" trunk serve --port "$COLDRAWDB_FRONTEND_PORT"
    ) >> "$frontend_log" 2>&1 &
