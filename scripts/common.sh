...
# persist-public-base-url：dev 双进程下邀请链接基址默认指向前端入口（deployment-plan §12.4），
# 否则 Host 推导会落到后端 :3000（无 SPA 路由）导致 /invite/{token} 404。
# 显式设置 PUBLIC_BASE_URL 优先（跨机验证时设为局域网地址）。
export PUBLIC_BASE_URL="${PUBLIC_BASE_URL:-http://127.0.0.1:${COLDRAWDB_FRONTEND_PORT}}"
