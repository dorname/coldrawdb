...
- **AC-14 ✓** — `GET /api/v1/diagrams/{id}` P95 < 300ms；
  实测 P95 = 1.9ms（20 sequential curl, `docs/phase4/perf/backend-perf-get.txt`）。
- **AC-15 ✓** — `PUT /api/v1/diagrams/{id}` P95 < 500ms；
  实测 P95 = 1.0ms（同上文件 `backend-perf-put.txt`）。
...
- **AC-23 ◐** — `rust_tti_p95 < react_baseline_tti_p95 × 1.1` 阈值 = 3500ms × 1.1 = 3850ms；
  baseline 测量见 `docs/phase4/perf/react-baseline-tti.txt`；rust TTI 实测待 Playwright/CI
  runner 补跑。
- **AC-24 ◐** — `rust_frame_p95 ≤ react_baseline_frame_p95` baseline = 22ms（P95 帧时间）；
  实测 `docs/phase4/perf/fps-w3.txt` / `fps-w4.txt` 写入待 CI runner。
- **AC-25 ◐** — `rust_heap < react_baseline_heap × 1.2` 阈值 = 95MB × 1.2 = 114MB；
  baseline 测量见 `docs/phase4/perf/react-baseline-heap.txt`。

> **AC-23/24/25 标注 ◐** = baseline 已闭环（react 侧 4 份文件存在 + 阈值回填 AC 完成），
> rust 侧 3 份实测文件待 dedicated runner 补齐；阈值形式已统一为「相对 React 基线」（plan
> §6.4 + Iteration 3 #1 修正），AC 描述无绝对值脱节。
...
- **§8 前端指标 3 项 ◐** = baseline 已闭环 + 阈值回填，rust 侧实测待 CI runner 补齐；
  不阻塞 React 下线 commit merge。
