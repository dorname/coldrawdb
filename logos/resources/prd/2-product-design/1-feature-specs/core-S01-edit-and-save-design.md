...
| 区域 | drawdb main 对应 | coldrawdb 原型锚点 |
|---|---|---|
| 顶栏工具 | `EditorHeader/ControlPanel.jsx` | `[data-testid="app-bar"]` |
| 左侧工具 | `FloatingControls.jsx` | `[data-testid="tool-rail"]` |
| 画布 | `EditorCanvas/Canvas.jsx` + `Table.jsx` | `[data-testid="editor-canvas"]` |
| 右侧 Inspector | `EditorSidePanel/SidePanel.jsx` 7 Tab | `[data-testid="inspector-panel"]` |
| 保存状态 | `SaveStateContext` + ControlPanel Spin/Tag | `[data-testid="save-state"]` / `[data-testid="revision-display"]` |
| 409 冲突 | （drawdb 客户端无服务端冲突；coldrawdb 新增） | `[data-testid="modal-conflict"]` |
| Command Palette | （drawdb main 无；E4 新增） | `[data-testid="command-palette"]` · `Ctrl+K` |
| Code View | main CodeEditor 只读 | `[data-testid="code-view-modal"]` · `[data-testid="btn-code-view"]` |
...
1. PUT 返回 409（原型演示：**双击** diagram 标题触发冲突模态）
2. 模态 `[data-testid="modal-conflict"]` 展示三选项：
   - **Reload** → GET 最新 diagram 覆盖本地
   - **Force** → PUT `expected_revision = current + 1`
   - **Cancel** → 关闭模态，保留本地
