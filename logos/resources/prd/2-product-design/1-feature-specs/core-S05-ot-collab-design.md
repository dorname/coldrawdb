...
└──────┬──────┘                 └────────┬────────┘                   └─────────────┘
       │ (ops 即保存)                 │ persist op_log
       │                              │ + 物化 doc_json
       ▼                              ▼
┌─────────────┐                 ┌─────────────────┐
│ actix-web   │   GET 物化读     │ SQLite          │
│ REST API    │◄────────────────│ operation_log   │
└─────────────┘                 └─────────────────┘
```

**与 S01 的关系**：room 模式下，**画布变更走 OT（即时）**；快照持久化由**服务端物化**承担（fix-collab-autosave-race：op 落库即物化 `doc_json`，客户端不再全量 PUT；PUT 对 room 图返回 409 USE_OP_CHANNEL），revision 即 head server_rev，**不再弹 S01 的 409 冲突模态**（OT 已合并并发 op）。
...
|---|---|---|---|
| 同步 | debounce PUT | 同 S01 | WS op 即时 + 服务端物化持久化（无客户端全量 PUT） |
