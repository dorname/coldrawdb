...
S07 数据字典 ── 随 S01 diagram JSON 快照持久化（migration 0007：diagram.dictionaries
              JSON 列 + field.dict_code 列；房间图经 doc_json 透出）；S05 下字典编辑
              作为模型 op 自然同步（协议零变更）；无新增 API 端点
```

S06 复用 diagram/bridge API，不直接依赖 SQLite，也不依赖 S03～S05 生产前端。当前 diagram API 尚未强制 S03 鉴权，因此 S06 MVP 仅限本地可信 stdio。

S07 复用 S01 持久化通路（diagram JSON 顶层新增可选 `dictionaries` 数组），不新增端点；持久化依赖 migration 0007 两列（规范化存储要求，见 core-01e §1）。
