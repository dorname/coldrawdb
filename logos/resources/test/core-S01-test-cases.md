...
| ST-S01-SS-01 | room-editor 可写角色 | 改表后等待自动保存 | fix-collab-autosave-race 起：room 模式 ops 即保存——编辑后 `save-state=dirty` + op 入队，**不产生全量 PUT**（无 saving 阶段）；dirty→saved 由 WS ack 驱动（ST-FE-S05-07/10 真实链路覆盖） | 本提案 C 批实现 |
...
| ST-S01-409-LOCAL-ONLY | 用户选择「仅本地编辑」 | 本地编辑 | fix-collab-autosave-race 起：不产生全量 PUT、**不出现 409 模态**（room 写只走 op 通道，PUT 一律 409 USE_OP_CHANNEL）；op 排队待重连 flush；须持续显示离线/409 风险文案 | 本提案 C 批实现 |
...
| ST-S01-409-LOCAL-ONLY | 仅本地编辑（无 PUT/409 模态） | S01+S05 | 本提案 C 批实现 |
