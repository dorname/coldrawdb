...
| ST-PU-27 | ADDED | 拖动 `splitter-inspector` 改变 Inspector 宽度且钳制 240–520 | 本提案（fix-canvas-zoom-invite-comment-resize）实现 |
| ST-PU-28 | ADDED | 窄视口（≤1179px）两条分隔条 `display:none` | 本提案（fix-canvas-zoom-invite-comment-resize）实现 |
| ST-PU-29 | ADDED | 拖动分隔条期间画布尺寸自动跟随（grid 重排 + RAF 尺寸比对，不重建 `#app`） | 本提案（fix-canvas-zoom-invite-comment-resize）实现 |
| ST-PU-30 | ADDED | ListView 树列宽拖动（钳制 160–420，刷新恢复） | 本提案（fix-canvas-zoom-invite-comment-resize）实现 |
...
## Splitter 分隔条用例（fix-canvas-zoom-invite-comment-resize）

> 编号续编：全局最大 UT-PU-21、ST-PU-26，本节自 UT-PU-22 / ST-PU-27 起编。规格来源：`core-09` §14。
