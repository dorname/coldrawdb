...
| ID | 前置 | 操作 | 预期 |
|---|---|---|---|
| ST-PU-01 [manual] | 断网 | 直接打开主 HTML | 无资源错误；登录页样式、SVG 图标和交互完整 |
| ST-PU-02 [manual] | 登录页 | 输入合法邮箱和 8 位密码并提交 | loading 后进入房间列表，出现欢迎 Toast |
| ST-PU-03 [manual] | 注册页 | 输入不一致密码后修正并提交 | 先显示字段错误；修正后进入房间列表 |
| ST-PU-04 [manual] | 房间列表 | 创建「数据模型评审」并提交 | 新房间出现并进入编辑器，AppBar 显示房间名 |
| ST-PU-05 [manual] | Editor 角色 | 新建表、添加字段、修改字段类型并拖拽表 | Canvas、Inspector、Activity 与保存状态一致 |
| ST-PU-06 [manual] | 已有两张表 | 开启关系工具，依次选择源/目标字段 | 画布出现关系线，关系计数与 Activity 更新 |
| ST-PU-07 [manual] | 已发生编辑 | 点击撤销、重做并等待自动保存 | 模型正确回退/恢复；状态从未保存→保存中→已保存，rev +1 |
| ST-PU-08 [manual] | 编辑器 | 打开导入，输入 SQL 并执行；再打开导出 | 新表生成；SQL/DBML/JSON 预览随模型更新 |
| ST-PU-09 [manual] | 编辑器 | 打开代码视图和命令面板，执行主题切换 | 浮层正常关闭；主题变化；无遗留 overlay |
| ST-PU-10 [manual] | Owner | 生成 Viewer 邀请，打开预览并接受 | 邀请 URL 可见；接受后以 Viewer 进入同一房间 |
| ST-PU-11 [manual] | Owner | 打开成员面板，将 Bob 改 Viewer 后移除 | 权限标签即时更新；确认后成员消失 |
| ST-PU-12 [manual] | 协作连接正常 | 模拟 Alice 光标与创建 `orders` | 光标移动；表、Activity、server rev 同步更新 |
| ST-PU-13 [manual] | 连接正常 | 模拟断线后本地修改，再恢复 | 队列计数增加；恢复后清零，出现同步成功反馈 |
| ST-PU-14 [manual] | 重连失败 | 选择仅本地编辑 | StatusBar 显示离线/409 风险；本地编辑仍可进行 |
| ST-PU-15 [manual] | Viewer | 点击新建表、修改字段、邀请成员 | 写操作被阻止并提示只读；远端操作仍可显示 |
| ST-PU-16 [manual] | 编辑器 | 模拟 Token 过期 | 会话指示进入续期并恢复，当前编辑状态不丢失 |
| ST-PU-17 [manual] | 桌面与 720px 视口 | 依次打开 Inspector、成员、IO 与模态 | 关键操作可达；内容无不可恢复遮挡 |
| ST-PU-18 [manual] | 系统 reduced-motion | 触发光标、Toast、抽屉 | 无非必要动画；信息仍完整可感知 |
