...
1. POST `/api/v1/diagrams` body 含 1 张表 + 2 字段 → 期望 200，验证 `id` 非空、`revision: 0`
...
5. DELETE `/api/v1/diagrams/{id}` → 期望 200
6. GET 再次 → 期望 404
...
> 更新于 feat-docker-compose-deploy：对齐 bridge 实际契约——SQL 解析在前端 WASM 侧完成，
> 后端 `/bridge/import/local` 接收结构化 payload（`{source?, payload: {name, tables:[...]}}`），
> 同步落库并写导入日志（无异步 worker 轮询）。**payload 内表/字段 ID 必须全局唯一**
> （`table`/`field` 主键全局，复用 ID 会静默冲突）。
...
1. POST `/api/v1/bridge/import/local` body:
   ```json
   {
     ...
     "payload": {
       "name": "Smoke Import",
       "tables": [{"id": "smk_t1", "name": "smoke_users",
                   "fields": [{"id": "smk_f1", "name": "id", "type": "INT", "primary": true},
                              {"id": "smk_f2", "name": "name", "type": "VARCHAR(255)", "not_null": true}]}]
     }
   }
   ```
   → 期望 200，验证 `data.diagram_id` / `data.log_id` 非空、`status: "success"`

2. GET `/api/v1/bridge/import/local/logs?limit=1` → 期望 200，验证 `data[0].id == log_id` 且 `status == "success"`

3. GET `/api/v1/diagrams/{diagram_id}` → 期望 200，验证 `tables[0].name == "smoke_users"`、`fields.length == 2`

4. 清理：DELETE `/api/v1/diagrams/{diagram_id}` → 期望 200
...
1. GET `/` → 期望 200，验证 body 含 `<div id="app">` 与 `<script type="module">`
...
1. POST `/api/v1/diagrams` body `{"name": "DB Health"}` → 期望 200
2. 直接查 staging 容器内 SQLite：
   ```sql
   SELECT name FROM sqlite_master WHERE type='table' ORDER BY name;
   ```
   验证返回包含 11 张核心业务表（migration 追加的表不影响判定；不含 `sqlite_sequence`）
3. 直接 `INSERT INTO diagram VALUES (...)` + 验证可读
4. DELETE 测试用 diagram

### 7.3 断言

- 至少包含 11 张核心表（migration 追加表不影响判定）：
