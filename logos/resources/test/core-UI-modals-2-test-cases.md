...
- 预期：
  ...
  2. 类型下拉清单为 PG **基类型**清单 = `UUID/INT/BIGINT/SERIAL/VARCHAR/TEXT/BOOLEAN/TIMESTAMP/NUMERIC`：含 UUID、BOOLEAN、SERIAL，**无 DATETIME**，无 `VARCHAR(255)`/`NUMERIC(10,2)` 参数化整串项（参数化口径见 core-01a §2.2，长度/小数位由独立编辑列配置）
  3. 当前类型基类型 UUID 已在清单内，**无占位追加**——下拉中 UUID 只出现一次（回归：修复前 PG 清单缺 UUID，兜底占位逻辑导致重复 UUID）
