...
## 变更提案
- 提案名称：fix-trunk-toml-case-conflict
- 提案目录：logos/changes/fix-trunk-toml-case-conflict/
...
- Delta 文件：`logos/changes/fix-trunk-toml-case-conflict/deltas/prd/3-technical-plan/3-deployment/core-01-deployment-plan.md`
...
8. 所有变更合并完成后，自动执行 git commit（告知用户，无需确认）：
   `git add -A && git commit -m "docs(fix-trunk-toml-case-conflict): merge spec deltas"`
9. 写入 `logos/changes/fix-trunk-toml-case-conflict/SPEC_MERGED`
   然后提示用户：按更新后的规格实现代码；代码完成后运行 `openlogos verify`；如无 `[deploy]` section，验收通过后由用户明确授权执行 `openlogos archive fix-trunk-toml-case-conflict`。
