# Delta — core-S07-data-dictionary.md（新增文件）

> 目标主文档：`logos/resources/prd/3-technical-plan/2-scenario-implementation/core-S07-data-dictionary.md`（新文件，merge 时整体创建）
...
- **API**：无新增。复用 S01 `PUT /api/v1/diagrams/{id}`（快照整体含 `dictionaries`）与 `GET ?share=`。
- **DB**：无新表。`dictionaries` 存于 diagram JSON 内部。
