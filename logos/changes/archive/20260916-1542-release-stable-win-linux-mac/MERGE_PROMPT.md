...
## 变更提案
- 提案名称：release-stable-win-linux-mac
- 提案目录：logos/changes/release-stable-win-linux-mac/
...
- Delta 文件：`logos/changes/release-stable-win-linux-mac/deltas/prd/3-technical-plan/3-deployment/core-01-deployment-plan.md`
- 目标目录：`logos/resources/prd/3-technical-plan/3-deployment/`
- 操作：读取 delta 中的 ADDED / MODIFIED / REMOVED 标记，合并到目标目录中对应的主文档

### 2. deltas/test/smoke/core-smoke-test-cases.md

- Delta 文件：`logos/changes/release-stable-win-linux-mac/deltas/test/smoke/core-smoke-test-cases.md`
...
8. 所有变更合并完成后，自动执行 git commit（告知用户，无需确认）：
   git add -A && git commit -m "docs(release-stable-win-linux-mac): merge spec deltas"
   然后提示用户：按更新后的规格实现代码，代码完成后运行 `openlogos verify` 验收，验收通过后明确授权执行 `openlogos archive release-stable-win-linux-mac`。
