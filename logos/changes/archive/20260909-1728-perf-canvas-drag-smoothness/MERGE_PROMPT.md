...
## 变更提案
- 提案名称：perf-canvas-drag-smoothness
- 提案目录：logos/changes/perf-canvas-drag-smoothness/
...
- Delta 文件：`logos/changes/perf-canvas-drag-smoothness/deltas/prd/2-product-design/1-feature-specs/core-01-editor-canvas.md`
- 目标目录：`logos/resources/prd/2-product-design/1-feature-specs/`
- 操作：读取 delta 中的 ADDED / MODIFIED / REMOVED 标记，合并到目标目录中对应的主文档

### 2. deltas/test/core-CR-canvas-test-cases.md

- Delta 文件：`logos/changes/perf-canvas-drag-smoothness/deltas/test/core-CR-canvas-test-cases.md`
...
8. 所有变更合并完成后，自动执行 git commit（告知用户，无需确认）：
   git add -A && git commit -m "docs(perf-canvas-drag-smoothness): merge spec deltas"
   然后提示用户：按更新后的规格实现代码，代码完成后运行 `openlogos verify` 验收，验收通过后明确授权执行 `openlogos archive perf-canvas-drag-smoothness`。
