...
# 路径：`logos/changes/add-frontend-completeness/deltas/test/core-CR-canvas-test-cases.md`
# 对齐参考源：tasks.md B3「画布渲染补全」+ `core-01-editor-canvas.md` §5.3 + `core-04-side-panel-tabs.md` §8 Issues Tab

> **背景**：现有 test-cases 编号体系无画布渲染（CR = Canvas Render）的 UT 编号。
> tasks.md B3 把 B3 范围挂在 S01 编号下（UT-S01-09/10），但 core-S01-test-cases.md §2 line 134/147
> 实际是后端 API（自动保存重试 + 字段类型校验），不适合画布渲染。本 delta 建立独立的
> CR 测试用例文件 + 与 `core-01-editor-canvas.md` §5.3 索引对齐。

## ADDED — 新文件 `logos/resources/test/core-CR-canvas-test-cases.md`

merge 时在 `logos/resources/test/` 下创建本文件。
