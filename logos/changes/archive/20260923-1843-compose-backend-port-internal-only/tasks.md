...
- [x] **一致性自检** — proposal 声明部署=是/smoke=是，本 section 已含部署方案与 smoke 用例 delta ✓

## [code] 代码实现

- [x] `docker-compose.yml` 移除 `3000:3000` 宿主映射（保留容器内 3000 与内部网络通信，healthcheck 走容器内 localhost:3000 不变）
- [x] `compose.release.yml` 同上移除宿主映射
- [x] `README.md` 健康检查段补限定语：compose 形态仅 9080 入口；直连 3000 仅适用单容器 `docker run`/开发模式
- [x] `scripts/smoke-local-scripts.sh` Stage 8（STABLE-01 执行段）同步增加宿主 3000 无监听断言（http_code 须为 000），与 smoke 用例规格对齐
