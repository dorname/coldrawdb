...
## 变更提案
- 提案名称：fix-collab-autosave-race
- 提案目录：logos/changes/fix-collab-autosave-race/
...
### 1. deltas/api/collab.yaml

- Delta 文件：`logos/changes/fix-collab-autosave-race/deltas/api/collab.yaml`
- 目标目录：`logos/resources/api/`
- 操作：读取 delta 中的 ADDED / MODIFIED / REMOVED 标记，合并到目标目录中对应的主文档

### 2. deltas/api/diagrams.yaml

- Delta 文件：`logos/changes/fix-collab-autosave-race/deltas/api/diagrams.yaml`
- 目标目录：`logos/resources/api/`
- 操作：读取 delta 中的 ADDED / MODIFIED / REMOVED 标记，合并到目标目录中对应的主文档

### 3. deltas/database/coldrawdb-v2-collab.sql

- Delta 文件：`logos/changes/fix-collab-autosave-race/deltas/database/coldrawdb-v2-collab.sql`
- 目标目录：`logos/resources/database/`
- 操作：读取 delta 中的 ADDED / MODIFIED / REMOVED 标记，合并到目标目录中对应的主文档

### 4. deltas/prd/3-technical-plan/2-scenario-implementation/core-S05-ot-collab.md

- Delta 文件：`logos/changes/fix-collab-autosave-race/deltas/prd/3-technical-plan/2-scenario-implementation/core-S05-ot-collab.md`
- 目标目录：`logos/resources/prd/3-technical-plan/2-scenario-implementation/`
- 操作：读取 delta 中的 ADDED / MODIFIED / REMOVED 标记，合并到目标目录中对应的主文档

### 5. deltas/test/core-S05-test-cases.md

- Delta 文件：`logos/changes/fix-collab-autosave-race/deltas/test/core-S05-test-cases.md`
...
8. 所有变更合并完成后，自动执行 git commit（告知用户，无需确认）：
   git add -A && git commit -m "docs(fix-collab-autosave-race): merge spec deltas"
   然后提示用户：按更新后的规格实现代码，代码完成后运行 `openlogos verify` 验收，验收通过后明确授权执行 `openlogos archive fix-collab-autosave-race`。
