...
批次 A — #19 拖动 click 穿透：
- [x] `frontend-rs/src/editor_render.rs`（+ 必要时 `editor_panels.rs` / styles）：有效拖动后抑制邀请 click；调整 pointer capture 释放时机
- [x] 批次 A 测试代码 + OpenLogos reporter（含 e2e：拖到 btn-invite 不打开 modal-invite）

批次 B — #20 表宽内容自适应：
- [x] `frontend-rs/src/editor_render.rs`：`compute_table_render_size` / measure 自适应；`0`/`None` = auto
- [x] 导入/建表 fit 路径（如适用）+ Set Table Width「0 = auto」语义对齐
- [x] 批次 B 测试代码 + OpenLogos reporter

批次 C — #21 线型 + 密度降噪：
- [x] 前端模型 + 后端 entity + migration：`line_type` / `stroke_style`
- [x] `editor_render.rs`：正交折线 / 直线 / 虚线渲染；选中态非相关线淡化
- [x] `editor_panels.rs`：关系 Inspector/浮层切换入口
- [x] 批次 C 测试代码 + OpenLogos reporter

批次 D — #22 表色 picker + 出边跟随：
- [x] `editor_panels.rs`：表色预设 + color picker / hex
- [x] `editor_render.rs`：`relation_stroke_color` 空色继承源表色
- [x] 批次 D 测试代码 + OpenLogos reporter
