...
首次全量验收暴露出预跑链路的历史缺陷：遗留关联查询测试依赖未初始化的工作区 SQLite 文件，预跑失败后又会把正式结果账本留在半写状态；同时部分 reporter ID 未以 CLI 可识别的表格形式登记。为保证本次原型修复能够被稳定验收，本变更追加隔离数据库、原子账本、用例索引和原型自动回归接入。

## 变更类型

设计与测试基础设施级（交互原型整合、视觉升级及验收链路加固；不改变生产 API、数据库 schema 或运行时业务逻辑）
...
- 影响的验收链路：`scripts/run-verify-tests.sh`、OpenLogos reporter 用例索引、遗留关联查询测试数据库隔离
...
- 是否需要部署：否
- 部署原因：仅修改 OpenLogos 文档、静态原型与测试基础设施，不进入运行时应用包
- 影响环境：本地评审
- 是否涉及数据迁移：否
- 是否需要回滚预案：否（Git 可直接回退原型与测试基础设施变更）
- 是否需要 smoke：否
...
验收预跑采用“备份正式账本 → 执行后端、前端与原型回归 → 校验已定义 ID/覆盖度 → 成功保留、失败恢复”的原子流程。关联查询测试使用唯一临时 SQLite 文件并初始化 schema，避免依赖工作区残留文件；既有标题式用例通过兼容索引登记为 CLI 可识别的表格用例。
