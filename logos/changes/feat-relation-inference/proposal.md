...
> Guard: `logos/.openlogos-guard` 指向 `feat-relation-inference`（外环代行开案）
> 内容基线：`.octos/proposals/draft-2026-09-02-product-batch/06-feat-relation-inference-proposal.md` v3（commit `493e33a`）
> 上游裁决链：黑板条目6 operator 批注（Q2=允许手动覆盖 + 字段按用户点击顺序）→ 外环判词 v1（语义层错误打回修订）→ 外环判词 v2（真值表方向颠倒打回修正）→ 外环判词 v3（采认）→ 外环代行开案
...
**外环判词修正（v2 语义层错误 + v3 方向颠倒）**：

- v2：草案 v1 误把 Q2 的 1/N 理解为**两端表的总字段数**（`fields.len()`）——这与关系基数**无语义关联**。反例：20 字段表与 3 字段表之间连**一条**单字段关系，草案会推导出 N+N→many_to_many，但用户只连了一对字段，应为 one_to_one
- v3：v2 真值表 `s>1 && e==1 → many_to_one`、`s==1 && e>1 → one_to_many`——**one_to_many / many_to_one 方向恰好颠倒**。正确语义：start_field 被多条关系引用（s>1）= start 是"一"侧 → `one_to_many`；end_field 被多条引用（e>1）= end 是"一"侧 → `many_to_one`

**Q2 正确语义（外环裁定，v3 修正后）**：「1+1→1:1、1+N→1:N、N+N→N:N」中的 1/N 指**该字段已参与的关系计数**（含本次新建），不是表总字段数。推导函数应改为 `infer_cardinality(start_field_id, end_field_id, store)`：
...
- 例：用户先连 A.a→B.x，再连 A.a→B.y——第二条创建时 A.a 参与 2 条（s=2）、B.y 参与 1 条（e=1）→ one_to_many（A 端一个字段对 B 端多个字段，语义是 A:B=1:N，start=A 时应得 one_to_many）
...
- `src/editor_panels.rs`：
  ...
  - `flip_reference_endpoints:439` —— 翻转端点函数（**推导后翻转需重新推导 cardinality**，基于翻转后的两端字段已参与关系计数，s/e 互换）
...
- 推导规则（operator Q2 裁决 + 外环判词修正 v2 + v3 方向颠倒修正）：**1/N 指该字段已参与的关系计数（含本次新建）**，不是表总字段数
...
| 决策 | 选 | 否 | 依据 |
|---|---|---|---|
| 推导依据 | 字段已参与关系计数（含本次新建）| 表总字段数 `fields.len()` | 外环判词修正 v2（语义层错误） |
| 推导规则 | s==1 && e==1 → one_to_one；s>1 && e==1 → one_to_many；s==1 && e>1 → many_to_one；s>1 && e>1 → many_to_many | 固定 one_to_many 默认 / 完全自动 / v2 方向颠倒 | operator Q2 裁决 + 外环判词修正 v2 + v3 方向颠倒修正 |
...
**实例推演**（users.id 双侧场景）：
- 先连 `users.id→orders.user_id`，再连 `users.id→order_items.user_id`：创建第二条时 s(users.id)=2、e(order_items.user_id)=1，语义 users:order_items = 1:N → **one_to_many**（v2 真值表给 many_to_one 错，v3 修正对）
- 反向：`orders.user_id→users.id`、`order_items.user_id→users.id`，创建第二条时 s=1、e=2 → **many_to_one**（v2 给 one_to_many 错，v3 修正对）
...
- **R2**：`flip_reference_endpoints` 翻转后重新推导 cardinality——翻转后 start/end 互换，s/e 互换，推导结果可能不同（如 s=1 && e=2 时 many_to_one → 翻转后 s=2 && e=1 时 one_to_many），需验证语义正确性
