...
# 路径：`logos/changes/add-frontend-completeness/deltas/test/core-SP-side-panel-test-cases.md`
# 对齐参考源：tasks.md B2「6 Tab + 搜索筛选」+ `core-04-side-panel-tabs.md` §11 测试 ID 索引

> **背景**：现有 `core-S01-test-cases.md` / `core-S02-test-cases.md` 是后端 API + data-access 层测试，
> 侧栏 Tab 的 UT/ST 编号（UT-SP-* / ST-SP-*）在 `core-04-side-panel-tabs.md` §11 索引中定义但
> 没有详细 test-cases 文件。tasks.md B2 错误地把 B2 范围挂在 S02 编号下，本 delta 建立
> 独立的 SP 测试用例文件 + 与 §11 索引对齐。

## ADDED — 新文件 `logos/resources/test/core-SP-side-panel-test-cases.md`
