...
// feat-room-recycle-bin-dropdown-and-db-export: recycle bin reads archived rows.
async fn load_archived_room(
    db: &DatabaseConnection,
    room_id: &str,
) -> Result<Option<RoomRow>, RoomsServiceError> {
    let row = db
        .query_one(Statement::from_sql_and_values(
            DatabaseBackend::Sqlite,
            "SELECT id, name, diagram_id, owner_id, created_at, updated_at FROM room WHERE id = ? AND archived_at IS NOT NULL LIMIT 1",
            vec![room_id.into()],
        ))
        .await
        .map_err(|e| RoomsServiceError::Db(DrawDBError::DatabaseError(e)))?;
...
// feat-room-recycle-bin-dropdown-and-db-export: recycle bin list (owner's archived rooms).
pub async fn list_archived_rooms(
    ...
    let count_row = db
        .query_one(Statement::from_sql_and_values(
            DatabaseBackend::Sqlite,
            "SELECT COUNT(1) AS c FROM room r WHERE r.owner_id = ? AND r.archived_at IS NOT NULL",
            vec![owner_id.into()],
        ))
        .await
        .map_err(|e| RoomsServiceError::Db(DrawDBError::DatabaseError(e)))?;
    ...
    let rows = db
        .query_all(Statement::from_sql_and_values(
            DatabaseBackend::Sqlite,
            "SELECT r.id, r.name, r.diagram_id, r.updated_at, r.archived_at, d.name AS diagram_title, (SELECT COUNT(1) FROM room_member m WHERE m.room_id = r.id) AS member_count FROM room r LEFT JOIN diagram d ON d.id = r.diagram_id WHERE r.owner_id = ? AND r.archived_at IS NOT NULL ORDER BY r.archived_at DESC LIMIT ? OFFSET ?",
            vec![owner_id.into(), (limit as i64).into(), (offset as i64).into()],
        ))
        .await
        .map_err(|e| RoomsServiceError::Db(DrawDBError::DatabaseError(e)))?;
...
// feat-room-recycle-bin-dropdown-and-db-export: restore an archived room (recycle bin).
pub async fn restore_room(
    db: &DatabaseConnection,
    room_id: &str,
    user_id: &str,
) -> Result<RoomRow, RoomsServiceError> {
    let room = load_archived_room(db, room_id)
        .await?
        .ok_or(RoomsServiceError::RoomNotFound)?;
    if room.owner_id != user_id {
        return Err(RoomsServiceError::Forbidden("仅房间 owner 可恢复房间".into()));
    }
    if let Some(existing) = active_room_for_diagram(db, &room.diagram_id).await? {
        return Err(RoomsServiceError::DiagramTaken {
            existing_room_id: existing,
        });
    }
    db.execute(Statement::from_sql_and_values(
        DatabaseBackend::Sqlite,
        "UPDATE room SET archived_at = NULL, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?",
        vec![room_id.into()],
    ))
    .await
    .map_err(|e| RoomsServiceError::Db(DrawDBError::DatabaseError(e)))?;
    load_active_room(db, room_id)
        .await?
        .ok_or(RoomsServiceError::Internal("room restore failed".into()))
}

// feat-room-recycle-bin-dropdown-and-db-export: hard delete an archived room (recycle bin).
// room_member / room_invite rows cascade via ON DELETE CASCADE; diagram is retained.
pub async fn permanent_delete_room(
    db: &DatabaseConnection,
    room_id: &str,
    user_id: &str,
) -> Result<(), RoomsServiceError> {
    let room = load_archived_room(db, room_id)
        ...
        .ok_or(RoomsServiceError::RoomNotFound)?;
    if room.owner_id != user_id {
        return Err(RoomsServiceError::Forbidden("仅房间 owner 可彻底删除房间".into()));
    }
    db.execute(Statement::from_sql_and_values(
        DatabaseBackend::Sqlite,
...
/// Derive the public base URL for invite links（core-S04 §7.2 / rooms.yaml InviteCreatedResponse）。
/// `PUBLIC_BASE_URL` env wins（trailing `/` trimmed）；否则由请求 Host header + scheme 推导，
/// `Forwarded` / `X-Forwarded-Proto` 优先于连接 scheme，非默认端口按原样保留。
/// 禁止硬编码 localhost / 127.0.0.1 / 无端口占位 host。
pub fn public_base_url(req: &HttpRequest) -> String {
    ...
    // Host header 为权威来源（含非默认端口）；缺省时回退连接信息
    let host = headers
        .get(header::HOST)
        .and_then(|v| v.to_str().ok())
        .map(str::to_string)
...
/// 从 RFC 7239 `Forwarded` 头解析 `proto=` 值
fn forwarded_proto(headers: &header::HeaderMap) -> Option<String> {
...
pub async fn create_invite(
    ...
    Ok(InviteCreated {
        invite_url: format!("{}/invite/{token}", public_base_url.trim_end_matches('/')),
        token,
        role: role.to_string(),
        expires_at,
    })
