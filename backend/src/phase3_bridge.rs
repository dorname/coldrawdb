...
use crate::bridge_introspect::{
    connect_error_message, connect_response_data, execute_ddl, execute_error_message, introspect,
...
pub fn phase3_bridge_routes(config: &mut web::ServiceConfig) {
    ...
    config.service(import_from_connection);
    config.service(export_execute_ddl);
}

/// feat-db-connect-import-and-ddl-realdb-verify：Bearer 校验（对齐 rooms_v1 bearer_user_id 口径）
fn bearer_check(req: &HttpRequest, request_id: &str) -> Result<(), HttpResponse> {
    let token = req
        .headers()
        .get("Authorization")
...
#[derive(Deserialize)]
struct ImportConnectReq {
    engine: String,
    source: String,
    /// fix-dbimport-save-and-pg-schema：仅 postgres 生效，缺省 public
    schema: Option<String>,
}

/// feat-db-connect-import-and-ddl-realdb-verify：连接数据库导入（core-03 §13）
/// fix-canvas-zoom-invite-comment-resize：响应由 DDL 文本改为结构化 tables JSON。
///
/// introspect 真实库 → 结构化 tables（persistence 同构，前端复用 JSON 导入管道消费），不落库、不写导入日志。
#[post("/bridge/import/connect")]
async fn import_from_connection(
    req: HttpRequest,
    body: web::Json<ImportConnectReq>,
) -> Result<HttpResponse, DrawDBError> {
    let request_id = next_id();
    if let Err(resp) = bearer_check(&req, &request_id) {
        return Ok(resp);
    }
    match introspect(&body.engine, &body.source, body.schema.as_deref()).await {
        Ok(tables) => Ok(HttpResponse::Ok().json(ApiResp {
            code: 0,
            data: connect_response_data(&body.engine, &tables),
            request_id,
        })),
        Err(err) => {
            let status = map_connect_status(&err);
            ...
            .json(ApiErr {
                code: status as i32,
                message: connect_error_message(&err).into(),
                request_id,
                details: None,
            }))
        }
...
#[derive(Deserialize)]
struct ExportExecuteReq {
    engine: String,
    source: String,
    ddl: String,
}

/// feat-room-recycle-bin-dropdown-and-db-export：连接数据库导出执行（core-03 §14）
///
/// 在目标真实库执行前端生成的 DDL；连接信息仅当次请求使用，不持久化。
#[post("/bridge/export/execute")]
async fn export_execute_ddl(
    req: HttpRequest,
    body: web::Json<ExportExecuteReq>,
) -> Result<HttpResponse, DrawDBError> {
    let request_id = next_id();
    if let Err(resp) = bearer_check(&req, &request_id) {
        return Ok(resp);
    }
    match execute_ddl(&body.engine, &body.source, &body.ddl).await {
        Ok(outcome) => Ok(HttpResponse::Ok().json(ApiResp {
            code: 0,
        ...
        Err(err) => {
            let status = map_execute_status(&err);
            Ok(HttpResponse::build(
                actix_web::http::StatusCode::from_u16(status)
                    .unwrap_or(actix_web::http::StatusCode::INTERNAL_SERVER_ERROR),
            )
            .json(ApiErr {
                code: status as i32,
                message: execute_error_message(&err),
                request_id,
                details: None,
            }))
        }
