    ...
    if (url.pathname.startsWith("/api/v1/diagrams/") && request.method() === "GET") {
      ...
      return response(route, 200, {
        ...
        data: {
          id: diagramId,
          name: "架构评审室",
          database: state.diagramCreates[0]?.database ?? null,
          revision: 0,
          tables: [],
          references: [],
          areas: [],
...
try {
  ...
  // ─── ST-S04-UI-09：创建房间选「新建空白模型」+ 引擎 MySQL → database 落账 + AppBar 跟随 ───
  await run(["ST-S04-UI-09"], "创建房间引擎前置选择透传 database", async page => {
    const state = await installApi(page);
    await login(page);
    await page.locator('[data-testid="btn-create-room"]').click();
    await page.locator('[data-testid="modal-create-room"]').waitFor();
    // 默认 diagram = 新建空白模型 → 引擎下拉必须可见
    const dbSelect = page.locator('[data-testid="create-room-database"]');
    await dbSelect.waitFor({ state: "visible" });
    await dbSelect.selectOption("mysql");
    await page.locator('[data-testid="create-room-name"]').fill("引擎评审室");
    await page.locator('[data-testid="create-room-submit"]').click();
    await page.locator('[data-testid="room-editor-page"]:visible').waitFor();

    assert.deepEqual(state.diagramCreates, [{ name: "引擎评审室", database: "mysql" }]);
    // 进入房间后 AppBar 引擎下拉 = mysql（mock 回显创建时的 database）
    await page.locator('[data-testid="app-db-select"]').waitFor();
    assert.equal(await page.locator('[data-testid="app-db-select"]').inputValue(), "mysql", "AppBar 引擎必须跟随新建图的 database");
  });

  // ─── ST-S04-UI-11：默认 Generic 不透传 database（回归不破） ───
  await run(["ST-S04-UI-11"], "创建房间默认 Generic 不透传 database", async page => {
    const state = await installApi(page);
    await login(page);
    await page.locator('[data-testid="btn-create-room"]').click();
    await page.locator('[data-testid="modal-create-room"]').waitFor();
    await page.locator('[data-testid="create-room-name"]').fill("默认引擎房间");
    await page.locator('[data-testid="create-room-submit"]').click();
    await page.locator('[data-testid="room-editor-page"]:visible').waitFor();

    assert.deepEqual(state.diagramCreates, [{ name: "默认引擎房间", database: null }], "默认 Generic 必须不透传 database");
  });
