...
fn expose_test_hooks(store: &EditorStore) {
    ...
    // ST-FE-S05-07~09 e2e 钩子：注入表走真实 store mutation（触发 diff watcher → op 上行），
    // 与读取画布状态（canvas 绘点无法被 Playwright DOM 断言）。仅 debug 构建暴露。
    let Some(win) = web_sys::window() else { return };
    let target: &js_sys::Object = win.unchecked_ref();

    let store_for_add = *store;
    let add_table = Closure::wrap(Box::new(move |name: String| -> String {
        let id = format!("t-e2e-{}", js_sys::Date::now() as u64);
        let offset = store_for_add.tables.with_untracked(|t| t.len()) as f64;
        let table = editor_core::types::Table {
            id: id.clone(),
            name,
            x: 160.0 + offset * 60.0,
            y: 160.0 + offset * 60.0,
        ...
        store_for_add.tables.update(|tables| tables.push(table));
        id
    }) as Box<dyn FnMut(String) -> String>);
    let _ = js_sys::Reflect::set(
        target,
        &JsValue::from_str("__cdb_debug_add_table"),
        add_table.as_ref().unchecked_ref(),
    );
    add_table.forget();

    let store_for_state = *store;
    let debug_state = Closure::wrap(Box::new(move || -> String {
    ...
    let _ = js_sys::Reflect::set(
        target,
        &JsValue::from_str("__cdb_debug_state"),
        debug_state.as_ref().unchecked_ref(),
    );
    debug_state.forget();
}
