...
pub mod types {
    ...
    pub struct Diagram {
        ...
        pub notes: Vec<Note>,
        pub areas: Vec<Area>,
        // feat-data-dictionary（S07）：数据字典数组。缺省/空数组序列化时跳过，
        // 旧图 JSON 无此字段反序列化为空数组（向后兼容）。
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
        pub dictionaries: Vec<DataDictionary>,
    }

    // feat-data-dictionary（S07）：PDManer 式代码映射字典。
    // 与后端 `DictionaryDto`/`DictionaryItemDto`（diagram_persistence.rs）形状对齐。
    #[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
    ...
    pub struct Field {
        ...
        #[serde(default)]
        pub tag: String,
        // feat-data-dictionary（S07）：字段绑定的数据字典编码（软引用，空串 = 未绑定）。
        // serde default = "" —— 老 JSON 无此字段反序列化为 ""，向后兼容。
        #[serde(default)]
    ...
    /// diagram-database-dialect：按引擎返回字段类型清单（UT-MM-36）。
    /// redesign-listview-type-length-canvas-fix：清单改为**裸基类型**（core-01a §2.2 参数化口径），
    /// 长度/小数位由 parse_field_type/compose_field_type 独立配置，不再出现 `VARCHAR(255)` 整串项。
    /// Sqlite/Mssql/Oracle 未在 UI 暴露，回落 Generic 清单。
    pub fn types_for_database(db: Database) -> &'static [&'static str] {
        const GENERIC: &[&str] = &["UUID", "INT", "BIGINT", "VARCHAR", "TEXT", "BOOLEAN"];
        const MYSQL: &[&str] = &["INT", "BIGINT", "VARCHAR", "TEXT", "DATETIME", "DECIMAL"];
...
/// 参数化类型解析结果（core-01a §2.2 口径，UT-MM-37）。
...
/// 将 `VARCHAR(32)` / `NUMERIC(10,2)` 这类整串拆为 基类型 + 长度 + 小数位。
/// 不匹配 `BASE(...)` 形态（含小写、非法参数）时整体视为 base（占位兼容，向后兼容老数据）。
pub fn parse_field_type(t: &str) -> ParsedFieldType {
    let t = t.trim();
    if let Some(open) = t.find('(') {
        if t.ends_with(')') && t[..open].chars().all(|c| c.is_ascii_uppercase()) {
            let inner = &t[open + 1..t.len() - 1];
            let mut parts = inner.splitn(2, ',');
            let len = parts.next().unwrap_or("").trim();
            let scale = parts.next().map(|s| s.trim()).unwrap_or("");
            let len_ok = !len.is_empty() && len.chars().all(|c| c.is_ascii_digit());
            let scale_ok = scale.is_empty() || scale.chars().all(|c| c.is_ascii_digit());
            if len_ok && scale_ok {
                return ParsedFieldType {
                    base: t[..open].to_string(),
                    len: len.to_string(),
    ...
    ParsedFieldType { base: t.to_string(), len: String::new(), scale: String::new() }
}

/// 按 core-01a §2.2 参数化口径合成类型整串：VARCHAR 带长度（缺省 255），
/// DECIMAL/NUMERIC 带 (长度,小数位)（缺省 10,2），其余基类型原样返回。
pub fn compose_field_type(base: &str, len: &str, scale: &str) -> String {
    match base {
        "VARCHAR" => format!("VARCHAR({})", if len.is_empty() { "255" } else { len }),
        "DECIMAL" | "NUMERIC" => format!(
            "{}({},{})",
            base,
            if len.is_empty() { "10" } else { len },
            if scale.is_empty() { "2" } else { scale }
        ),
        _ => base.to_string(),
    }
}
...
impl EditorStore {
    ...
    pub fn load(&self, diagram: Diagram) {
        ...
        // feat-data-dictionary（S07）：缺省为空数组（serde default 已保证），
        // 加载时规范化悬空绑定（dict_code 指向不存在字典 → 置空，EX-7.4）。
        let dicts = diagram.dictionaries;
        let mut tables = self.tables.get();
        crate::editor_dict::sanitize_dangling_bindings(&mut tables, &dicts);
        self.tables.set(tables);
        self.dictionaries.set(dicts);
    ...
    pub fn snapshot(&self, id: String, name: String) -> Diagram {
        Diagram {
            ...
            dictionaries: self.dictionaries.get(),
        }
    }
}
...
pub enum Command {
    ...
    DeleteField {
        table_id: String,
        field_id: String,
        /// redesign-listview-type-length-canvas-fix：删除时字段快照 + 原下标，
        /// 支持撤销恢复（ST-SP-LIST-02 删字段 Ctrl+Z 恢复）；旧构造点传 None 保持 V1 行为
        field: Option<Field>,
        index: Option<usize>,
    },
    ...
    /// feat-data-dictionary（S07，core-01e §6）：字典域前后快照命令。
    /// 字典 CRUD / 字典项编辑 / 绑定 / 改编码联动 / 删除级联统一记录 before→after；
    /// 级联置空与字典删除同处一个 before/after 对 → 单次 undo 单元（UT-S07-06 / ST-S07-05）。
    DictSnapshot {
        before: Vec<DataDictionary>,
        after: Vec<DataDictionary>,
...
fn apply_dict_state(
    store: &EditorStore,
    dicts: &[DataDictionary],
    bindings: &[(String, String, String)],
) {
    store.dictionaries.set(dicts.to_vec());
    let mut tables = store.tables.get();
    for (tid, fid, code) in bindings {
        if let Some(f) = tables
            .iter_mut()
            .find(|t| t.id == *tid)
            .and_then(|t| t.fields.iter_mut().find(|f| f.id == *fid))
        {
            f.dict_code = code.clone();
        }
    }
...
impl CommandStack {
    ...
    pub fn revert(store: &EditorStore, cmd: &Command) -> Result<(), CoreError> {
        match cmd {
            ...
            Command::DeleteField { table_id, field_id, field, index } => {
                // redesign-listview-type-length-canvas-fix：有快照时按原下标恢复（撤销删除）
                if let Some(field) = field {
                    ...
                    let table = tables
                        ...
                        .find(|t| t.id == *table_id)
                        .ok_or_else(|| CoreError::new(format!("table '{}' not found", table_id)))?;
                    let pos = index.unwrap_or(table.fields.len()).min(table.fields.len());
                    table.fields.insert(pos, field.clone());
                    store.tables.set(tables);
                } else {
...
mod tests {
    ...
    fn test_delete_field_happy_and_missing_ut_s01_07() {
        ...
        assert!(CommandStack::apply(&store, &mut stack, Command::DeleteField {
            table_id: "t1".into(), field_id: "f1".into(), field: None, index: None,
        }).is_ok());
        // 字段不存在 → Err
        let r = CommandStack::apply(&store, &mut stack, Command::DeleteField {
            table_id: "t1".into(), field_id: "missing".into(), field: None, index: None,
        });
    ...
    /// UT-MM-36: types_for_database 三组**基类型**清单（core-01a §2.2 参数化口径，
    /// redesign-listview-type-length-canvas-fix 改为裸基类型，不含 VARCHAR(255) 等整串项）
    #[test]
    fn ut_mm_36_types_for_database() {
        use crate::editor_core::types::{types_for_database, Database};
        // Generic 组 6 项含 UUID/VARCHAR（回归不破 + 占位防重）
        assert_eq!(
            types_for_database(Database::Generic),
            &["UUID", "INT", "BIGINT", "VARCHAR", "TEXT", "BOOLEAN"]
        );
        // MySQL 组：含 DATETIME/DECIMAL 裸基类型，无 SERIAL
        let mysql = types_for_database(Database::Mysql);
        assert!(mysql.contains(&"DATETIME"), "UT-MM-36: MySQL 必须含 DATETIME");
        assert!(mysql.contains(&"DECIMAL"), "UT-MM-36: MySQL 必须含 DECIMAL");
        assert!(!mysql.contains(&"SERIAL"), "UT-MM-36: MySQL 不得含 SERIAL");
        // PostgreSQL 组：含 UUID/BOOLEAN/SERIAL/TIMESTAMP/NUMERIC 裸基类型，无 DATETIME
        let pg = types_for_database(Database::Postgresql);
        assert!(pg.contains(&"UUID"), "UT-MM-36: PostgreSQL 必须含 UUID");
        assert!(pg.contains(&"BOOLEAN"), "UT-MM-36: PostgreSQL 必须含 BOOLEAN");
        assert!(pg.contains(&"SERIAL"), "UT-MM-36: PostgreSQL 必须含 SERIAL");
        assert!(pg.contains(&"TIMESTAMP"), "UT-MM-36: PostgreSQL 必须含 TIMESTAMP");
        assert!(pg.contains(&"NUMERIC"), "UT-MM-36: PostgreSQL 必须含 NUMERIC");
        assert!(!pg.contains(&"DATETIME"), "UT-MM-36: PostgreSQL 不得含 DATETIME");
        // 清单均为裸基类型，不含参数化整串
        for db in [Database::Generic, Database::Mysql, Database::Postgresql] {
            for t in types_for_database(db) {
                assert!(!t.contains('('), "UT-MM-36: 清单必须为裸基类型，发现整串 {t}");
            }
        }
        // 未暴露引擎（Sqlite/Mssql/Oracle）回落 Generic 清单
        assert_eq!(types_for_database(Database::Sqlite), types_for_database(Database::Generic));
        assert_eq!(types_for_database(Database::Mssql), types_for_database(Database::Generic));
        assert_eq!(types_for_database(Database::Oracle), types_for_database(Database::Generic));
    }

    /// UT-MM-37: parse_field_type / compose_field_type 纯函数测试（core-01a §2.2 参数化口径）
    #[test]
    fn ut_mm_37_parse_compose_field_type() {
        use crate::editor_core::{compose_field_type, parse_field_type};
        // parse：VARCHAR 长度
        let p = parse_field_type("VARCHAR(32)");
        assert_eq!((p.base.as_str(), p.len.as_str(), p.scale.as_str()), ("VARCHAR", "32", ""));
        // parse：NUMERIC 精度 + 小数位
        let p = parse_field_type("NUMERIC(10,2)");
        assert_eq!((p.base.as_str(), p.len.as_str(), p.scale.as_str()), ("NUMERIC", "10", "2"));
        // parse：裸基类型
        let p = parse_field_type("UUID");
        assert_eq!((p.base.as_str(), p.len.as_str(), p.scale.as_str()), ("UUID", "", ""));
        // parse：非法串整体视为 base（小写 / 非数字参数 / 不闭合括号）
        assert_eq!(parse_field_type("varchar(32)").base, "varchar(32)");
        assert_eq!(parse_field_type("VARCHAR(abc)").base, "VARCHAR(abc)");
        assert_eq!(parse_field_type("VARCHAR(32").base, "VARCHAR(32");
        // compose：缺省值
        assert_eq!(compose_field_type("VARCHAR", "", ""), "VARCHAR(255)");
        assert_eq!(compose_field_type("VARCHAR", "32", ""), "VARCHAR(32)");
        assert_eq!(compose_field_type("NUMERIC", "", ""), "NUMERIC(10,2)");
        assert_eq!(compose_field_type("DECIMAL", "12", "4"), "DECIMAL(12,4)");
        assert_eq!(compose_field_type("UUID", "", ""), "UUID");
        assert_eq!(compose_field_type("INT", "11", ""), "INT");
        // 往返一致
        let p = parse_field_type("VARCHAR(32)");
        assert_eq!(compose_field_type(&p.base, &p.len, &p.scale), "VARCHAR(32)");
        let p = parse_field_type("NUMERIC(10,2)");
        assert_eq!(compose_field_type(&p.base, &p.len, &p.scale), "NUMERIC(10,2)");
